...
def _squared_distance_to_line(first_point, last_point, points):
    ...
    if np.all(last_point == 0):
        ...
        squared_distances = np.sum(points * points, 1) - np.square(
...
def rdp(points, threshold):
    ...
    if n_dims > n_points:
        print(
            "Number of dimensions appears to be greater than the number of elements; input may be transposed"
        )
    ...
    while np.any(active):
        ...
        first = (
            _fnz(~active[first:-1] & active[first + 1 :]) + first
        )  # The first endpoint is an inactive point just before an active point
        last = _fnz(~active[(first + 1) :]) + (
            first + 1
        )  # The last endpoint is the next inactive point after the first endpoint

        between = slice((first + 1), last)

        square_distances = _squared_distance_to_line(
            points[first, :], points[last, :], points[between, :]
        )
...
def main():
    ...
    points = np.column_stack(
        [test_x / XSPAN * AXES_HEIGHT, test_y / YSPAN * AXES_WIDTH]
    )
    ...
    plt.title("Retained {:0.2%} of points".format(keep.sum() / keep.size))
