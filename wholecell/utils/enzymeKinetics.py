...
class EnzymeKinetics(object):
    ...
    def __init__(
        self, reactionRateInfo, kcatsOnly=False, useCustoms=True, moreThanKcat=False
    ):
        ...
        if kcatsOnly:
            ...
            for constraintID, reactionInfo in self.reactionRateInfo.items():
                ...
                if (
                    len(reactionInfo["kM"])
                    or len(reactionInfo["kI"])
                    or reactionInfo["customRateEquation"]
                ):
                    continue
    ...
    def checkKnownSubstratesAndEnzymes(
        self,
        metaboliteSMatrixNamesNoCompartment,
        metaboliteNamesWithConcentrations,
        enzymeNames,
        ...
        for constraintID, reactionInfo in self.reactionRateInfo.items():
            ...
            if reactionType == "standard":
                ...
                raise Exception(
                    "Reaction type '%s' is unknown. Must be either 'standard' or 'custom'."
                    % (reactionType,)
                )
        ...
        unknownVals = {
            "unknownSubstrates": unknownSubstrates,
            "unknownEnzymes": unknownEnzymes,
            "unknownCustomVars": unknownCustomVars,
        }

        return knownConstraints, unusableConstraints, unknownVals

    def reactionRate(
        self, reactionInfo, metaboliteConcentrationsDict, enzymeConcentrationsDict
    ):
        if reactionInfo["rateEquationType"] == "standard":
            return self.reactionStandard(
                reactionInfo, metaboliteConcentrationsDict, enzymeConcentrationsDict
            )
        elif reactionInfo["rateEquationType"] == "custom":
            return self.reactionCustom(
                reactionInfo, metaboliteConcentrationsDict, enzymeConcentrationsDict
            )
        else:
            raise NameError(
                "rateEquationType %s not recognized! Must be either 'standard' or 'custom'."
                % (reactionInfo["rateEquationType"])
            )

    def reactionStandard(
        self, reactionInfo, metaboliteConcentrationsDict, enzymeConcentrationsDict
    ):
        # Find the enzymes needed for this rate
        enzymeConc = enzymeConcentrationsDict[reactionInfo["enzymeIDs"][0]].asNumber(
        ...
        if len(kMs) + len(kIs) > len(substrateIDs):
            raise enzymeKineticsError(
                "The number of saturation constants (kMs and kIs) must not be greater than the number of substrates for a standard reaction. For constraint {}, there are {} kMs, {} kIs, and {} substrates. ReactionInfo: {}".format(
                    reactionInfo["constraintID"],
                    len(kMs),
                    len(kIs),
                    len(substrateIDs),
                    reactionInfo,
        ...
        for kM in reactionInfo["kM"]:
            substrateConc = metaboliteConcentrationsDict[substrateIDs[idx]].asNumber(
                COUNTS_UNITS / VOLUME_UNITS
            )
    ...
    def reactionCustom(
        ...
        for customParameter in customParameters[len(customParameterConstants) :]:
            ...
            if variable in enzymeConcentrationsDict:
                parametersArray.append(
                    enzymeConcentrationsDict[variable].asNumber(
                        COUNTS_UNITS / VOLUME_UNITS
                    )
                )
        ...
        return (COUNTS_UNITS / TIME_UNITS / VOLUME_UNITS) * customRateLaw(
            *parametersArray
        )

    def allConstraintsDict(
        self, metaboliteConcentrationsDict, enzymeConcentrationsDict
    ):
        constraintsDict = {}
        for constraintID, reactionInfo in self.reactionRateInfo.items():
            constraintsDict[constraintID] = self.reactionRate(
                reactionInfo, metaboliteConcentrationsDict, enzymeConcentrationsDict
            )
    ...
    def allReactionsDict(self, metaboliteConcentrationsDict, enzymeConcentrationsDict):
        ...
        for constraintID, reactionInfo in self.reactionRateInfo.items():
            ...
            reactionsDict[reactionID][constraintID] = self.reactionRate(
                reactionInfo, metaboliteConcentrationsDict, enzymeConcentrationsDict
            )
    ...
    def ratesView(
        ...
        for idx, reactionID in enumerate(reactionIDs):
            if reactionID in reactionsToConstraintsDict:
                ...
                coefficient = (
                    reactionsToConstraintsDict[reactionID]["coefficient"]
                    if "coefficient" in reactionsToConstraintsDict[reactionID]
                    else 1
                )
                if constraintID in self.reactionRateInfo:
                    rates[idx] = coefficient * self.reactionRate(
                        self.reactionRateInfo[constraintID],
                        metaboliteConcentrationsDict,
                        enzymeConcentrationsDict,
                    )
                else:
        ...
        if len(unknownConstraints) > 0:
            raise Exception(
                "No rate estimate found for the following {} constraintIDs {}.".format(
        ...
        if len(unknownReactions) > 0:
            raise Exception(
                "No rate estimate found for the following {} reactionIDs {}.".format(
    ...
    def ratesViewConstraints(
        ...
        if not self.inputsChecked:
            knownConstraints, unusableConstraints, unknownVals = (
                self.checkKnownSubstratesAndEnzymes(
                    metaboliteConcentrationsDict,
                    enzymeConcentrationsDict,
                    enzymeNames={},
                    removeUnknowns=False,
            ...
            if len(unusableConstraints) > 0:
                raise Exception(
                    "Unable to compute kinetic rate for these reactions: {}\n. Missing values for: {}".format(
                        list(unusableConstraints.keys()), unknownVals
                    )
                )
        ...
        if len(unknownConstraints) > 0:
            raise Exception(
                "No rate estimate found for constraintIDs {}.".format(
                    unknownConstraints
                )
            )
    ...
    def ratesViewReactions(
        ...
        constraintsDict = self.allConstraintsDict(
            metaboliteConcentrationsDict, enzymeConcentrationsDict
        )
        for idx, reactionID in enumerate(reactionIDs):
    ...
    def ratesViewLowest(
        ...
        """
        return self.ratesViewReactions(
            reactionIDs,
            metaboliteConcentrationsDict,
            enzymeConcentrationsDict,
            np.amin,
