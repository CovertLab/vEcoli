...
class NetworkFlowCPLEX(NetworkFlowProblemBase):
    ...
    def setFlowMaterialCoeff(self, flow, material, coefficient):
        if self._eqConstBuilt:
            ...
            self._model.linear_constraints.set_coefficients(
                list(zip([materialIdx], [flowIdx], [coefficient]))
            )
        else:
    ...
    def setFlowObjectiveCoeff(self, flow, coefficient):
        ...
        if self.quadratic_objective:
            self._model.objective.set_quadratic_coefficients(idx, idx, coefficient)
        else:
            self._model.objective.set_linear(idx, coefficient)
    ...
    def getShadowPrices(self, materials):
        ...
        materials = [
            self._materialIdxLookup.get(material, None) for material in materials
        ]
    ...
    def buildEqConst(self):
        ...
        self._model.linear_constraints.add(
            rhs=np.zeros(nMaterials), senses="E" * nMaterials
        )
        self._model.linear_constraints.set_coefficients(
            list(zip(row.tolist(), col.tolist(), data.tolist()))
        )
