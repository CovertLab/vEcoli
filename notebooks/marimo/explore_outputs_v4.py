...
def _(
    ...
    if bulk_sp_plot.value and plot_df_bulk is not None:
...
def _(
    ...
    chart_mrna = None
    if mrna_select_plot.value and plot_df_mrna is not None:
        _n = len(active_datasets)
        _base = (
...
def _(
    ...
    chart_monomers = None
    if monomer_select_plot.value and plot_df_monomers is not None:
        _n = len(active_datasets)
        _base = (
...
def _(
    ...
    select_rxns = mo.ui.multiselect(options=_opts, value=_default, max_selections=500)
    return (select_rxns,)
...
def _(
    ...
    if select_rxns.value:
        ...
        if _kind == "catalyst":
            ...
            _display = list(select_rxns.value)
...
def _(
    ...
    chart_rxns = None
    if select_rxns.value and plot_df_rxns is not None:
        _n = len(active_datasets)
...
def _(scalar_cols):
    ...
    _prefixes = (
        "listeners__mass__",
        "listeners__rnap_data__",
        "listeners__ribosome_data__",
    )
    physiology_classified = {}
    physiology_label_to_col = {}
    for _col in sorted(c for c in scalar_cols if c.startswith(_prefixes)):
        _cat, _lbl = _classify(_col)
        # Avoid label collisions across categories — prefix the column suffix
        # if a label is already taken.
        _key = (
            _lbl
            if _lbl not in physiology_label_to_col
            else f"{_lbl} [{_col.rsplit('__', 1)[-1]}]"
        )
        physiology_classified.setdefault(_cat, []).append((_col, _key))
        physiology_label_to_col[_key] = _col
    ...
    physiology_categories = [c for c in _category_order if c in physiology_classified]
    return physiology_categories, physiology_classified, physiology_label_to_col
...
def _(mo):
    about_physiology_md = mo.md(
        "Time course of scalar physiology quantities. Use **category** to "
        "scope the quantity picker to compatible units — masses (fg), "
        "fractions / fold-changes (dimensionless), volume (L), growth rates, "
        "machinery snapshot counts (active RNAPs / ribosomes), per-step event "
        "counters (initiations / terminations / stalls), and effective "
        "elongation rates each get their own category so you don't end up "
        "plotting a 1.0 fraction next to a 400 fg cell mass on the same axis. "
        "Display labels include units; the raw `listeners__…` column names "
        "are still what's queried under the hood."
    )
...
def _(
    ...
    if physiology_select.value and plot_df_physiology is not None:
        _n = len(active_datasets)
        _base = (
...
def _(
    ...
    if transcription_label_type.value == "gene name":
        ...
        _opts = mrna_gene_ids
    transcription_select_plot = mo.ui.multiselect(
        options=_opts,
        value=mrna_override(select_pathway.value),
        max_selections=500,
    )
...
def _(
    ...
    if transcription_select_plot.value:
...
def _(
    ...
    if transcription_select_plot.value and plot_df_transcription is not None:
        _n = len(active_datasets)
...
def _(
    ...
    translation_select_plot = mo.ui.multiselect(
        options=_opts,
        value=_default,
        max_selections=500,
    )
...
def _(
    ...
    if translation_select_plot.value:
        _listener, _kind = translation_quantities[translation_quantity_select.value]
        if _kind == "monomer":
...
def _(
    ...
    if translation_select_plot.value and plot_df_translation is not None:
        _n = len(active_datasets)
...
def _():
    # Display name → (listener_column, index_kind).
    # index_kind: "cistron" → all cistrons; "tu" → all TUs.
    regulation_quantities = {
        "TF count bound (per cistron)": (
            "listeners__rna_synth_prob__n_bound_TF_per_cistron",
            "cistron",
        ),
        "TF count bound (per TU)": (
...
def _(mo, regulation_quantities):
    regulation_quantity_select = mo.ui.dropdown(
        ...
        value="TF count bound (per cistron)",
    )
...
def _(
    ...
    regulation_select_plot = mo.ui.multiselect(
        options=_opts,
        value=_default,
        max_selections=500,
    )
...
def _(mo):
    about_regulation_md = mo.md(
        "Time course of bound transcription-factor counts per cistron or per "
        "TU. Use to see *why* a gene's expression changed: a jump in bound "
        "TF count typically precedes a change in transcription rate (see "
        "**Transcription** tab) and downstream mRNA / protein counts. The "
        "entity picker spans **all** cistrons / TUs (not just mRNAs), since "
        "the underlying listeners track TF binding across the full RNA set. "
        "Pathway selection above auto-populates with mRNA-cistron entries; "
        "non-matching entries are dropped silently."
    )
...
def _(
    ...
    if regulation_select_plot.value:
        _listener, _kind = regulation_quantities[regulation_quantity_select.value]
        if _kind == "cistron":
...
def _(
    ...
    if regulation_select_plot.value and plot_df_regulation is not None:
        _n = len(active_datasets)
...
def _(np, pl):
    def get_plot_df(
        ...
        labels) instead of `item_selector_ui.value`. Lets a consumer translate
        ...
        _keys = display_labels if display_labels is not None else item_selector_ui.value
