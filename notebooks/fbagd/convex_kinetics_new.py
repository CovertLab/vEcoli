...
class ConvexKineticsNew:
    ...
    def set_up_variables(self, S_matrix, R_matrix, flow_data):
        ...
        S_s_nz, S_p_nz, S_i_nz, S_a_nz = [
            np.array(ar.nonzero()) for ar in [S_s, S_p, S_i, S_a]
        ]
        ...
        print(
            f"Number of metabolites: {n_met}, number of reactions: {n_rxn}, number of flux sets: {n_flux_set}",
            f"Number of Km_s: {n_Km_s}, number of Km_p: {n_Km_p}, number of Km_i: {n_Km_i}, number of Km_a: {n_Km_a}",
            f"Number of concentrations: {c.shape}, number of y_f: {y_f.shape}, number of y_r: {y_r.shape}",
            sep="\n",
        )
    ...
    def construct_binding_matrix(
        ...
        d_alpha, d_beta = (
            np.zeros(n_alpha, dtype=np.int64),
            np.zeros(n_beta, dtype=np.int64),
        )
        ...
        for i in range(n_rxn):
            ...
            if Sr is not None:
                idx_s_cur_rxn = np.concatenate(
                    (rxn_s_nz == i, rxn_i_nz == i, rxn_a_nz == i)
                )
                idx_p_cur_rxn = np.concatenate(
                    (rxn_p_nz == i, rxn_i_nz == i, rxn_a_nz == i)
                )
            else:
            ...
            s_sat_perm = np.array(
                list(itertools.product([0, 1], repeat=sum(idx_s_cur_rxn)))
            )[1:, :]
            p_sat_perm = np.array(
                list(itertools.product([0, 1], repeat=sum(idx_p_cur_rxn)))
            )[1:, :]
        ...
        print(
            f"Shape of C_alpha: {C_alpha.shape}, shape of C_beta: {C_beta.shape}",
            f"Shape of d_alpha: {d_alpha.shape}, shape of d_beta: {d_beta.shape}",
            sep="\n",
        )

        return C_alpha, C_beta, d_alpha, d_beta

    def construct_kinetic_objective(
        self,
        flow_data,
        n_flux_set,
        n_rxn,
                ...
                LSE_expr.append(
                    cp.hstack(
                        [
                            lvE[j, i]
                            + expr_sat_alpha
                            - cp.multiply(np.ones(n_term_s), denom),
                            lvE[j, i]
                            + expr_sat_beta
                            - cp.multiply(np.ones(n_term_p), denom),
                            lvE[j, i] + 0 - cp.multiply(np.ones(1), denom),
                            cp.multiply(np.ones(1), num)
                            - cp.multiply(np.ones(1), denom),
                        ]
                    )
                )
    ...
    def create_objective_function(
        ...
        l1 = cp.sum(cp.hstack([cfwd, crev, cp.vec(c)])) + cp.sum(
            cp.hstack([-Km_s, -Km_p])
        )  # regularization
        prior = cp.norm1(cp.hstack([cfwd, crev, cp.vec(c)])) + cp.norm1(
            cp.hstack([-Km_s, -Km_p])
        )  # regularization
    ...
    def set_parameter_bounds(
        self, cfwd, crev, c, Km_s, Km_p, Km_i, Km_a, lower_bound=-12, upper_bound=12
    ):
        constr = [
            cp.hstack([cfwd, crev, cp.vec(c), Km_s, Km_p]) >= lower_bound,
    ...
    def add_mechanistic_constraints(
        self,
        constr,
        flow_data,
        K_eq,
        ...
        for i, r in enumerate(S.T):
            ...
            haldane.append(
                ...
                == cfwd[i]
                - crev[i]
                + r[S_p_idx] @ Km_p[Km_p_idx]
                - (-r[S_s_idx]) @ Km_s[Km_s_idx]
            )

        for j in range(n_flux_set):
            constr.extend(
                [
                    cp.multiply(S.T @ cp.vec(c[:, j]), sign[j, :])
                    <= cp.multiply(K_eq, sign[j, :])
                ]
            )
