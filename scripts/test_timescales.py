...
def _get_michaelis_menten_derivative(
    rate_constant, michaelis_constant, hill_coefficient, enzyme_conc
):
    def derivative(y0, _):
        return (
            -rate_constant
...
def _get_fickian_derivative(diffusivity, external_conc, volume, inner_bias=1):
    def derivative(y0, _):
...
ODES = {
    "cephaloridine": {
        ...
        "Efflux": (
            _get_michaelis_menten_derivative(
                param_store.get(("cephaloridine", "efflux", "kcat"))
                .to(1 / units.sec)
                .magnitude,
                param_store.get(("cephaloridine", "efflux", "km"))
                .to(units.mM)
                .magnitude,
                param_store.get(("cephaloridine", "efflux", "n")),
                param_store.get(("concs", "initial_pump")).to(units.mM).magnitude,
            ),
            # Assume diffusion is fast.
            param_store.get(("cephaloridine", "mic")).to(units.mM).magnitude,
        ),
        "Hydrolysis": (
            _get_michaelis_menten_derivative(
                param_store.get(("cephaloridine", "hydrolysis", "kcat"))
                .to(1 / units.sec)
                .magnitude,
                param_store.get(("cephaloridine", "hydrolysis", "km"))
                .to(units.mM)
                .magnitude,
                param_store.get(("cephaloridine", "hydrolysis", "n")),
                param_store.get(("concs", "initial_hydrolase")).to(units.mM).magnitude,
            ),
            # Assume diffusion is fast.
            param_store.get(("cephaloridine", "mic")).to(units.mM).magnitude,
        ),
        "Diffusion": (
            _get_fickian_derivative(
                (
                    ...
                    * param_store.get(("cephaloridine", "permeability", "outer"))
                )
                .to(units.L / units.sec)
                .magnitude,
                param_store.get(("cephaloridine", "mic")).to(units.mM).magnitude,
                (
    ...
    "ampicillin": {
        ...
        "Efflux": (
            _get_michaelis_menten_derivative(
                ...
                .to(1 / units.sec)
                .magnitude,
                param_store.get(("ampicillin", "efflux", "km")).to(units.mM).magnitude,
                param_store.get(("ampicillin", "efflux", "n")),
                param_store.get(("concs", "initial_pump")).to(units.mM).magnitude,
            ),
            # Assume diffusion is fast.
            param_store.get(("ampicillin", "mic")).to(units.mM).magnitude,
        ),
        "Hydrolysis": (
            _get_michaelis_menten_derivative(
    ...
    "tetracycline": {
        "Efflux": (
            _get_michaelis_menten_derivative(
                param_store.get(("tetracycline", "efflux", "kcat"))
                .to(1 / units.sec)
                .magnitude,
                param_store.get(("tetracycline", "efflux", "km"))
                .to(units.mM)
                .magnitude,
                param_store.get(("tetracycline", "efflux", "n"))
                .to(units.count)
                .magnitude,
                param_store.get(("concs", "initial_pump")).to(units.mM).magnitude,
            ),
            # Assume diffusion is fast.
            param_store.get(("tetracycline", "mic")).to(units.mM).magnitude,
        ),
        "Outer Diffusion": (
            _get_fickian_derivative(
                (
                ...
                .magnitude,
                param_store.get(("tetracycline", "mic")).to(units.mM).magnitude,
                param_store.get(("shape", "initial_periplasm_volume"))
                .to(units.L)
                .magnitude,
                np.exp(
                    (
        ...
        "Inner Diffusion": (
            _get_fickian_derivative(
                (
                    param_store.get(("shape", "initial_outer_area"))
                    * param_store.get(("tetracycline", "permeability", "inner"))
                )
                .to(units.L / units.sec)
                .magnitude,
                ...
                .to(units.mM)
                .magnitude,
                param_store.get(("shape", "initial_cytoplasm_volume"))
                .to(units.L)
                .magnitude,
            ),
            0,
...
def main():
    for duration in (0.1, 10, 1000):
        fig, axes = plt.subplots(nrows=len(ODES) * 2, figsize=(6.4, 4.8 * len(ODES)))
        ...
        for i, (plot_name, ode_definitions) in enumerate(ODES.items()):
            ...
            for label, (derivative_func, y0) in ode_definitions.items():
                ...
                ax2.plot(t, (y - y[0]) / (y[-1] - y[0]), label=label, alpha=0.5)
            ax1.legend()
            ax2.legend()
