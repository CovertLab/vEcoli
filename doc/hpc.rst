...
To ensure reproducibility, workflow steps on Sherlock are intentionally constrained to nodes
with CPUs that have been tested to produce numerically identical output for a 4-seed,
4-generation baseline glucose workflow. This is necessary because older CPUs lack support
for features like AVX-512 instructions that NumPy and OpenBLAS can utilize on newer CPUs.
As such, older CPUs tend to use different code paths in NumPy and OpenBLAS, leading to
unpredictable floating point differences. See ``sherlock --> nf_config_overrides --> CLUSTER_OPTIONS``
under the source code for :py:data:`runscripts.workflow.CLUSTER_PRESETS` for the current
list of supported CPU generations. As new CPU models are introduced, they will need to be
tested for consistency with existing ones before being added to this list.
...
.. warning::
  ...
  for details. Be sure to use the ``--constraint`` flag to request nodes with
  supported CPU generations as described in :ref:`reproducibility`.
...
resources should be submitted as SLURM batch scripts to the ``owners``
partition (see :ref:`sherlock-noninteractive`).
...
.. warning::
   As of Sep 19, 2026, the ``mcovert`` partition does not contain any nodes with
   a CPU generation supported by ``workflow.py`` (see :ref:`reproducibility`).
   Scripts manually run on this partition will diverge from results generated by
   ``workflow.py``.
...
If not using the deprecated ``mcovert`` partition, also include the
``#SBATCH --constraint=<supported_cpu_generation>`` directive to request
nodes with a supported CPU generation as described in :ref:`reproducibility`.
...
.. code-block:: bash
   ...
   #SBATCH --constraint="CPU_GEN:BGM|CPU_GEN:SIE|CPU_GEN:GEN|CPU_GEN:SPR"
