...
.. note::
  ...
  :py:mod:`runscripts.parca`. If run as part of a workflow, downstream analysis
  steps require the validation data pickle to be saved as ``validationData.cPickle``
  in the same directory as the specified sim data.
...
- ``validation_data_path``: List of string paths to validation data pickle files
  ...
  passed as is to analysis script ``plot`` functions. Defaults to empty list.
...
- ``cpus``: Number of CPU threads to let DuckDB use. DuckDB generally scales well
  with more threads at the cost of proportionally increased RAM usage (default: 1)
...
1. ``variant_data_dir``
   ...
   List of string paths to one or more directories containing
   variant simulation data pickles and metadata saved by :py:mod:`runscripts.create_variants`.
   Must provide exactly one path for each experiment ID in ``experiment_ID`` and in the
   same order. This option is strongly recommended when analyzing data generated by
   :py:mod:`runscripts.workflow`.

2. ``sim_data_path`` AND ``variant`` / ``variant_range``
  
   ``sim_data_path`` is a list of string paths to simulation data pickle files, one
   corresponding to each variant ID in ``variant`` or ``variant_range``. Analysis
   scripts will receive empty ``variant_metadata`` / ``variant_names`` dictionaries
   and a ``sim_data_dict`` that maps each variant in ``variant`` or ``variant_range``
   to each simulation data file in ``sim_data_path``. This option is mainly meant for
   analyzing one-off simulations run with :py:mod:`runscripts.sim` /
   :py:mod:`ecoli.experiments.ecoli_master_sim`.

3. ``sim_data_path``, ``variant_metadata_path``, AND ``variant`` / ``variant_range``

   ``sim_data_path`` is a list of string paths to simulation data pickle files, one
   corresponding to each variant ID in ``variant`` or ``variant_range``.
   ``variant_metadata_path`` is a string path to a metadata JSON in
   the same format as the ``metadata.json`` saved by :py:mod:`runscripts.create_variants`
   (``{"variant name": {"variant ID 1": {"param 1": ..., ...}, ...}}``,
   see :ref:`variant_output`). This is a middle ground between the above options that
...
Here are additional configuration options specific to workflows
(capitalization important, defaults in ``configs/default.json``):

- Under ``parca_options``:

  - ``memory_gb`` (integer): Baseline amount of RAM (in GB) to allocate
    to the ParCa. This option has no effect when running
    :py:mod:`runscripts.parca` manually.
  - ``slurm_time_hrs`` (integer): Amount of wall time (in hours) to allocate
    for the ParCa when running on a SLURM cluster. This option has no effect
    when running :py:mod:`runscripts.parca` manually or on a non-SLURM system.

- Under ``analysis_options``:

  - ``memory_gb`` (integer): Baseline amount of RAM (in GB) to allocate
    to each analysis job in the workflow. This option has no effect when
    running :py:mod:`runscripts.analysis` manually, as DuckDB will use
    all available RAM on the system.
  - ``slurm_time_hrs`` (integer): Amount of wall time (in hours) to allocate
    for each analysis job when running on a SLURM cluster. This option has no
    effect when running :py:mod:`runscripts.analysis` manually or on a non-SLURM system.
  - ``cpus`` (integer): While this option controls the number of DuckDB CPU
    threads when running :py:mod:`runscripts.analysis` manually, it
    controls the number of CPU cores allocated to each analysis job when
    specified in a workflow config.
  - ``duckdb_threads`` (integer): In workflows, we let users specify the
    number of DuckDB CPU threads separately from allocated CPU cores because
    performance can improve by using more threads than physical cores when
    `querying remote files <https://duckdb.org/docs/stable/guides/performance/how_to_tune_workloads#querying-remote-files>`_
    (e.g. Google Cloud Storage). If not set, defaults to the value of ``cpus``.

- ``SIM_CPUS`` (integer): Number of CPU cores to allocate to each
  simulation job in the workflow. At the moment, anything higher than 2
  just leads to wasted resources. Under specific conditions (HPC clusters
  that allocate CPU threads instead of full cores, systems where filesystem
  bandwidth is more of a bottleneck than CPU performance), setting this to 2
  can improve performance because, by default, the Parquet emitter will
  attempt to write sim outputs in a non-blocking background
  thread while the sim continues to execute. See the ``threaded`` option
  under ``emitter_arg`` in :ref:`json_config`.
- ``SIM_MEM`` (integer): Amount of RAM (in GB) to allocate for each sim.
- ``SIM_TIME`` (integer): Amount of wall time (in hours) to allocate for
  each sim. This value is only enforced when running on a SLURM cluster.
- ``HYPERQUEUE`` (boolean): If True, run the workflow using HyperQueue
  to orchestrate jobs. This is helpful on SLURM clusters where a large
  batch of resources can be allocated at the start of a workflow and
  then used to run many smaller jobs over time without having to
  re-queue each job individually. Note that only simulation jobs are
  run using HyperQueue; other steps in the workflow are not (e.g. ParCa,
  variant creation, analysis) to allow for more resource flexibility.
- ``HQ_CORES`` (integer): Number of CPU cores to allocate to HyperQueue
  workers when running with HyperQueue.
...
- ``{experiment ID}``: Folder in output directory named the experiment ID
  ...
  without overwriting data as long as they have different experiment IDs.
    ...
    - ``analysis``: Hive-partitioned (with experiment ID partition omitted because
      ...
      to the analysis name and type (e.g. ``multigeneration`` analysis output for the
      ``new_gene_counts`` analysis will be in sub-folders of the format
      ``variant={}/lineage_seed={}/plot/analysis=new_gene_counts``). Each analysis
      folder also contains a ``metadata.json`` file with the analysis type, name,
      and parameters used to generate the output in that folder.
    - ``nextflow``: Nextflow-related files.
        ...
        - ``workflow_config.json``: Configuration JSON passed to
          :py:mod:`runscripts.workflow` after resolving the ``inherit_from`` option
          and loading default values.
        - ``workflow_config_stripped.json``: Same as above but with resource
          allocation options (e.g., ``SIM_MEM``, ``parca_options --> cpus``) removed.
          This is the config JSON actually passed to Nextflow, allowing users to change
          resource allocation options when resuming a workflow without triggering
          re-execution of already completed tasks.
