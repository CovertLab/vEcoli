...
class GrowthData(object):
    def __init__(self, kb):
        self.tau_d = np.array(
            kb.cellDryMassComposition["doublingTime"].asNumber(units.min)
        )

        avgToBeginningConvFactor = kb.avg_cell_to_initial_cell_conversion_factor
        self._dryMass = (
            np.array([148.0, 258.0, 433.0, 641.0, 865.0]) / avgToBeginningConvFactor
        )  # TOKB
        self._proteinMass = (
            self._dryMass * kb.cellDryMassComposition["proteinMassFraction"]
        )
        ...
        self.RRNA23S_MASS_SUB_FRACTION = (
            0.525  # This is the fraction of RNA that is 23S rRNA
        )
        self.RRNA16S_MASS_SUB_FRACTION = (
            0.271  # This is the fraction of RNA that is 16S rRNA
        )
        self.RRNA5S_MASS_SUB_FRACTION = (
            0.017  # This is the fraction of RNA that is 5S rRNA
        )
        ...
        self.proteinMassParams, *_ = curve_fit(
            exp2, self.tau_d, self._proteinMass, p0=p0
        )
        ...
        self.C_PERIOD = 40.0  # TOKB. [minutes]
        self.D_PERIOD = 20.0  # TOKB. [minutes]
    ...
    def _chromMass(self, kb):
        ...
        dntMasses = (
            kb.get_masses(kb.molecule_groups.polymerizedDNT_IDs) / kb.n_avogadro
        ).asUnit(units.g)
    ...
    def dnaMass(self, tau_d):
        ...
        return self.chromMass * (
            1
            + 1 * (np.maximum(0.0, self.CD_PERIOD - tau_d) / self.C_PERIOD)
            + 2 * (np.maximum(0.0, self.CD_PERIOD - 2 * tau_d) / self.C_PERIOD)
            + 4 * (np.maximum(0.0, self.CD_PERIOD - 4 * tau_d) / self.C_PERIOD)
        )
