...
def load_regulon_db_file(filename: str) -> List[List[str]]:
    ...
    if not os.path.exists(path):
        raise IOError(
            f"{path} does not exist. Run {REGULON_DB_SCRIPT} to download regulonDB data."
        )
...
def load_ecocyc_tf_gene_interactions(
    ...
    with open(ECOCYC_REGULATION_FILE) as f:
        ...
        for line in reader:
            ...
            add_tf_gene_data(
                tf_genes, tf, gene, effect, positive, negative, unknown, split, verbose
            )
...
def load_tf_gene_interactions(
    ...
    if ecocyc and regulondb:
        raise NotImplementedError(
            "Using both EcoCyc and RegulonDB requires reconciling differences in TF/gene IDs."
        )

    tf_genes = {}
    if ecocyc:
        tf_genes.update(
            load_ecocyc_tf_gene_interactions(valid_types, split=split, verbose=verbose)
        )
        if verbose:
    ...
    if regulondb:
        tf_genes.update(
            load_regulondb_tf_gene_interactions(split=split, verbose=verbose)
        )
        if verbose:
...
def load_regulation(
    directory: str, genes: np.ndarray, tfs: np.ndarray
) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def calculate_fold_changes(
    ...
    fcs = A * (
        high_P.sum(0) / np.sum(high_P != 0, 0) - low_P.sum(0) / np.sum(low_P != 0, 0)
    )
...
def compare_wcm_nca(
    nca_pairs: Dict[str, Dict[str, float]],
...
def match_statistics(
    ...
    print(
        f"Negative regulation matches: {correct_neg}/{annotated_neg} ({percent_match_neg:.1f}%)"
    )
    print(
        f"Positive regulation matches: {correct_pos}/{annotated_pos} ({percent_match_pos:.1f}%)"
    )
    ...
    error_predictions = np.sqrt(
        np.mean(((E[~no_prediction] - E_est[~no_prediction]) / E[~no_prediction]) ** 2)
    )
    print(f"\nError fitting original data: {error:.3f}")
    print(
        f"Error fitting original data (excluding unpredicted samples): {error_predictions:.3f}"
    )
    print(
        f"No prediction made: {n_zeros}/{n_samples} ({100 * n_zeros / n_samples:.1f}%)"
    )
    print(
        f"Negative prediction made: {n_negatives}/{n_samples} ({100 * n_negatives / n_samples:.1f}%)"
    )
    ...
    pearson_consistent = stats.pearsonr(
        wcm_fcs[wcm_consistent], nca_fcs[wcm_consistent]
    )
    if np.any(nonzero):
    ...
    print(
        f"Only if WCM consistent: r={pearson_consistent[0]:.3f} (p={pearson_consistent[1]:.0e}, n={np.sum(wcm_consistent)})"
    )
    print(
        f"Excluding unmatched: r={pearson_no_zeros[0]:.3f} (p={pearson_no_zeros[1]:.0e}, n={np.sum(nonzero)})"
    )
...
def plot_results(
    ...
    def plot(ax, series, label, hist_range, n_bins, color):
        ...
        ax.hist(
            series, color=color, bins=n_bins, range=hist_range, alpha=0.5, label=label
        )
    ...
    # TF regulation types
    positive_tfs = np.array(
        [np.all([v > 0 for v in tf_genes.get(tf, {}).values()]) for tf in tfs]
    )
    negative_tfs = np.array(
        [np.all([v < 0 for v in tf_genes.get(tf, {}).values()]) for tf in tfs]
    )
    ...
    if np.any(nonzero):
        ...
        pearson_no_zeros = [np.nan, np.nan]
    ax_wcm.plot(
        wcm_fcs[wcm_consistent], nca_fcs[wcm_consistent], "x", label="WCM consistent"
    )
    ax_wcm.plot(
        wcm_fcs[~wcm_consistent],
        nca_fcs[~wcm_consistent],
        "x",
        label="WCM inconsistent",
    )
    ...
    ax_wcm.set_title(
        f"All: r={pearson_all[0]:.3f} (p={pearson_all[1]:.0e}, n={len(wcm_fcs)})\n"
...
if __name__ == "__main__":
    ...
    if args.analysis is None:
        ...
        no_cache = not (
            os.path.exists(network_cache_file) and os.path.exists(tf_cache_file)
        )
        ...
        if args.force or no_cache or args.iterative:
            ...
            initial_tf_map, tfs = create_tf_map(
                b_numbers, synonyms, tf_genes, verbose=args.verbose
            )

            if not args.iterative:
                initial_tf_map, tfs = nca.nca_criteria_check(
                    initial_tf_map, tfs, verbose=args.verbose
                )
        else:
        ...
        if args.sigma_factors:
            tfs, initial_tf_map = add_sigma_factors(
                tfs, b_numbers, synonyms, mapping=initial_tf_map, verbose=args.verbose
            )
        if args.noise:
            tfs, initial_tf_map = add_noisy_expression(
                tfs, *args.noise, mapping=initial_tf_map
            )  # type: ignore

        # Solve NCA problem
        nca_method = getattr(nca, args.method)
        if args.iterative:
            A, P, tfs = nca.iterative_sub_nca(
                nca_method,
                seq_data,
                initial_tf_map,
                tfs,
                statistics=match_statistics,
                statistics_args=(tf_genes, gene_symbols),
                n_iters=args.iterative_iterations,
                splits=args.iterative_splits,
                parallel=args.parallel,
                robust_iters=args.robust_iterations,
                status_step=args.status,
                verbose=args.verbose,
            )
            np.save(tf_cache_file, tfs)
        else:
            A, P = nca_method(
                seq_data,
                initial_tf_map,
                n_iters=args.robust_iterations,
                status_step=args.status,
            )
