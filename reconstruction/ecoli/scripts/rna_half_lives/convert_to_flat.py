...
# Files
CHEN_INPUT = os.path.join(
    DATA_DIR, "msb145794-sup-0009-supp_table_s4.csv"
)  # Supplementary Table S4
BERNSTEIN_INPUT = os.path.join(
    DATA_DIR, "3181Table5.csv"
)  # Supporting Table 5, saved as .csv (originally .xls) and comments removed
GENES_FILE = os.path.join(ROOT_PATH, "reconstruction", "ecoli", "flat", "genes.tsv")
...
def compare_half_lives(
    ...
    plt.scatter(overlapping_bernstein_half_lives, overlapping_chen_half_lives, s=2)
    plt.xlim([0, 35])
    plt.ylim([0, 35])
