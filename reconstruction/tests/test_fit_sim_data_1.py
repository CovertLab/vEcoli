...
class Test_fitkb1(unittest.TestCase):
    def test_totalCountFromMassesAndRatios(self):
        ...
        self.assertEqual(count, 80.0)

        # Test normal call with units
        totalMass = 10.0 * units.fg
    ...
    def test_proteinDistributionFrommRNA(self):
        ...
        proteinDist = proteinDistributionFrommRNA(
            distribution_mRNA, np.ones(3) / 3, netLossRate
        )
        ...
        np.testing.assert_array_almost_equal_nulp(
            expectedDistribution.asNumber(), proteinDist
        )
    ...
    def test_calculateMinPolymerizingEnzymeByProductDistributionRNA(self):
        ...
        nMin = calculateMinPolymerizingEnzymeByProductDistribution(
    ...
    def test_netLossRateFromDilutionAndDegradationProtein(self):
        ...
        net = netLossRateFromDilutionAndDegradationProtein(
            doublingTime, degradationRates
        )
        net.asUnit(1 / units.h)  # Check units are 1/time
        self.assertEqual(
            net.asNumber(1 / units.min).tolist(),
            (
                (np.log(2) / doublingTime).asNumber(1 / units.min)
                + degradationRates.asNumber(1 / units.min)
            ).tolist(),
        )
