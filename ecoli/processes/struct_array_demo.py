    ...
    def dict_value_updater(current, update):
        ...
        for operation in update.get("add_items", {}):
            ...
            if not set(state.keys()).issubset(defaults.keys()):
                raise Exception(
                    "Attempted to write state with keys not included in defaults"
                )
...
class StructArrayDemo(Process):
    ...
    DTYPE = np.dtype(
        [
            (
                "_key",
                "<U16",
            ),  # necessary for _add, _delete operations. We restrict keys to be 16 characters or less.
            ("unique_index", "int"),
            ("domain_index", "int"),
            ("coordinates", "int"),
            ("direction", "bool"),
            ...
            return {
                "active_RNAPs": {
                    ...
                    "_updater": make_dict_value_updater(
                        unique_index=0, domain_index=0, coordinates=0, direction=True
                    ),
                    "_emit": True,
                }
            }
    ...
    def next_update(self, timestep, states):
        ...
        if self.time_til_next_delete <= 0:
            ...
            self.time_til_next_delete = self.random.exponential(1 / self.rate_delete)

        return update


def test_struct_array_updater(
    mode="struct_array",
    rate_add=0.5,
    rate_delete=0.2,
    mean_add=1,
    ...
    if mode == "default" or mode == "dict_value":
        ...
        initial_state["active_RNAPs"] = np.empty(
            (0, len(StructArrayDemo.DTYPE)), dtype=StructArrayDemo.DTYPE
        )
    ...
    for i, rate_ratio in enumerate(sweep["add_to_delete_rate_ratio"]):
        for j, total_time in enumerate(sweep["total_time"]):
            ...
            for k, mode in enumerate(sweep["mode"]):
                time, data[mode] = test_struct_array_updater(
                    mode=mode,
            ...
            for time, snapshot in data["default"].items():
                ...
                for idx, rnap in enumerate(snapshot["active_RNAPs"].values()):
                    assert all(
                        array_from(rnap) == np.array(struct_array_snapshot[idx][1:])
                    )
    ...
    for i, rate_ratio in enumerate(sweep["add_to_delete_rate_ratio"]):
        for j, mode in enumerate(sweep["mode"]):
            axs[j].plot(
                ...
                result[j, i, :],
                label=rate_ratio,
                color=(1 - i / n_lines, i / n_lines, 0.25),
            )
            axs[j].set_title(f'Runtime using\n"{mode}"')
