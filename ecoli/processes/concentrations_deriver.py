...
class ConcentrationsDeriver(Step):
    ...
    def next_update(self, timestep, states):
        if self.bulk_var_idx is None:
            self.bulk_var_idx = bulk_name_to_idx(
                self.bulk_var, states['bulk']['id'])
        ...
        bulk_concs = (bulk_counts * units.count / AVOGADRO / 
                      volume).to(units.millimolar)
        new_concs = {
            **var_concs, **dict(zip(self.bulk_var, bulk_concs))
        }
        update = {
