...
class RnaSynthProb(Step):
    ...
    def __init__(self, parameters=None):
        ...
        self.old_tf_ids = self.parameters["old_tf_ids"]
        self.new_tf_ids = self.parameters["new_tf_ids"]
        self.cistron_ids = self.parameters["cistron_ids"]
        self.n_TU = len(self.rna_ids)
        self.n_old_TF = len(self.old_tf_ids)
        self.n_new_TF = len(self.new_tf_ids)
            ...
            "rna_synth_prob": listener_schema(
                {
                    ...
                    "old_n_bound_TF_per_TU": (
                        [[0] * self.n_old_TF] * self.n_TU,
                        self.rna_ids,
                    ),
                    "old_n_bound_TF_per_cistron": ([], self.cistron_ids),
                    "new_n_bound_TF_per_TU": (
                        [[0] * self.n_new_TF] * self.n_TU,
                        self.rna_ids,
                    ),
                    "new_n_bound_TF_per_cistron": ([], self.cistron_ids),
                    "total_rna_init": 0,
                }
            ),
    ...
    def next_update(self, timestep, states):
        ...
        return {
            "rna_synth_prob": {
                ...
                "old_n_bound_TF_per_cistron": self.cistron_tu_mapping_matrix.dot(
                    states["rna_synth_prob"]["old_n_bound_TF_per_TU"]
                )
                .astype(np.int16)
                .T,
                "new_n_bound_TF_per_cistron": self.cistron_tu_mapping_matrix.dot(
                    states["rna_synth_prob"]["new_n_bound_TF_per_TU"]
                )
                .astype(np.int16)
                .T,
            }
