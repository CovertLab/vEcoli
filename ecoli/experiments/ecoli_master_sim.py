...
def parse_variants(sim_data: 'SimulationDataEcoli', 
    ...
    """
    Parse variants and parameters specified under ``variant`` key of config.
        ...
        variant_config: Mapping of variant names to dictionary of the form::

            {
                'params': {
                    ...
                    'param_name': {
                        # Each parameter has only one of the following keys
                        'value': [...]
                        'linspace': {
    ...
    for variant_name, variant_params in variant_config.items():
        ...
        parsed = {}
        combo = False
        for param_name, param_conf in variant_params.items():
            if param_name == 'combo':
                combo = param_conf
                continue
            if len(param_conf) > 1:
