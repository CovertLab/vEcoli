...
def test_equilibrium():
    ...
    utest_partioned_molecules = mannwhitneyu(wc_d_molecules, [-p for p in moleculeChanges], alternative="two-sided")
    utest_rates = mannwhitneyu(rates, wc_rxnEvolve, alternative="two-sided")


    percent_error_threshold = 0.05
    import  ipdb; ipdb.set_trace()
    molecule_partition_error = percent_error(sum(wc_d_molecules), sum(moleculeChanges))
    rates_error = percent_error(sum(rates), sum(wc_rxnEvolve))
    ...
    assert utest_rates.pvalue < utest_threshold, (
        "Distribution of rxnFluxes is different between wcEcoli and vivarium-ecoli"
    ...
    assert rates_error < percent_error_threshold, (
        f"Total # of rates differs between wcEcoli and vivarium-ecoli"
