...
def mrna_scatter_comparison(data, config, out_dir="out"):
    ...
    bulk = data["bulk"]
    mRNA_data = {
        mol_id: series for mol_id, series in bulk.items() if mol_id in mRNA_ids
    }
    ...
    expectedCounts = (
        expectedCountsArbitrary / expectedCountsArbitrary.sum() * counts.sum()
    )
...
def get_masses_from_sim(data, sim_data):
    ...
    molecular_weights = {
        molecule_id: sim_data.getter.get_mass([molecule_id]).asNumber(
            units.fg / units.mol
        )[0]
    ...
    smallMolecules = sum_lists_to_array(
        [
            mass_from_counts_array(bulk[mol_id], molecular_weights.get(mol_id))
            for mol_id in smallMolecule_ids
            if mol_id in bulk
        ]
    )
...
def mass_fractions(data, config, out_dir="out"):
    ...
    plt.plot(t / 60.0, cell, linewidth=2)
    plt.plot([t[0] / 60.0, t[-1] / 60.0], [2 * cell[0], 2 * cell[0]], "r--")
    plt.ylabel("Total Mass (fg)")
    plt.title("Total Mass Final:Initial = %0.2f" % (cell[-1] / cell[0]), fontsize=8)

    plt.subplot(n_subplots, 1, 2)
    plt.plot(t / 60.0, cellDry, linewidth=2)
    plt.plot([t[0] / 60.0, t[-1] / 60.0], [2 * cellDry[0], 2 * cellDry[0]], "r--")
    plt.ylabel("Dry Mass (fg)")
    plt.title("Dry Mass Final:Initial = %0.2f" % (cellDry[-1] / cellDry[0]), fontsize=8)

    ax = plt.subplot(n_subplots, 1, 3)
    plt.plot(t / 60.0, protein, linewidth=2)
    plt.plot([t[0] / 60.0, t[-1] / 60.0], [2 * protein[0], 2 * protein[0]], "r--")
    plt.ylabel("Protein Mass (fg)")
    plt.title(
        "Total Protein Mass Final:Initial = %0.2f\nAverage dry mass fraction: %0.3f"
        % (protein[-1] / protein[0], np.mean(protein / cellDry)),
        fontsize=8,
    )
    ax2 = ax.twinx()
    ax2.plot(t / 60.0, protein / cellDry, second_axis_color)
    ...
    plt.plot(t / 60.0, rna, linewidth=2)
    plt.plot([t[0] / 60.0, t[-1] / 60.0], [2 * rna[0], 2 * rna[0]], "r--")
    plt.ylabel("RNA Mass (fg)")
    plt.title(
        "Total RNA Mass Final:Initial = %0.2f\nAverage dry mass fraction: %0.3f"
        % (rna[-1] / rna[0], np.mean(rna / cellDry)),
        fontsize=8,
    )
    ax2 = ax.twinx()
    ax2.plot(t / 60.0, rna / cellDry, second_axis_color)
    ax2.yaxis.set_major_formatter(FormatStrFormatter("%.3f"))
    ax2.set_yticks(ax2.get_ylim())
    ax2.set_ylabel("Fraction of dry mass", color=second_axis_color)

    ax = plt.subplot(n_subplots, 1, 5)
    plt.plot(t / 60.0, smallMolecules, linewidth=2)
    plt.plot(
        [t[0] / 60.0, t[-1] / 60.0],
        [2 * smallMolecules[0], 2 * smallMolecules[0]],
        "r--",
    )
    plt.ylabel("Small molecules (fg)")
    plt.title(
        "Total Small Molecule Mass Final:Initial = %0.2f\nAverage dry mass fraction: %0.3f"
        % (smallMolecules[-1] / smallMolecules[0], np.mean(smallMolecules / cellDry)),
        fontsize=8,
    )
    ax2 = ax.twinx()
    ax2.plot(t / 60.0, smallMolecules / cellDry, second_axis_color)
    ax2.yaxis.set_major_formatter(FormatStrFormatter("%.3f"))
    ax2.set_yticks(ax2.get_ylim())
    ax2.set_ylabel("Fraction of dry mass", color=second_axis_color)
