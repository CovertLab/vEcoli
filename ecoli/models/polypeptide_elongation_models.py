...
class SteadyStateElongationModel(TranslationSupplyElongationModel):
    ...
    def __init__(self, parameters, process):
        ...
        self.maxRibosomeElongationRate = float(self.parameters[
            'elongation_max'].asNumber(units.aa / units.s))
        ...
        self.ppgpp_synthesis_reaction = self.parameters[
            'ppgpp_synthesis_reaction']
        self.ppgpp_degradation_reaction = self.parameters[
            'ppgpp_degradation_reaction']
        self.synthesis_index = self.ppgpp_reaction_names.index(
            self.ppgpp_synthesis_reaction)
        self.degradation_index = self.ppgpp_reaction_names.index(
        ...
        self.aa_charging_mask = np.array([aa not in aa_removed_from_charging
    ...
    def request(self, timestep, states, aasInSequences):
        ...
        if self.process.ppgpp_regulation:
            ...
            ppgpp_conc = self.counts_to_molar * counts(states['bulk'], 
                self.process.ppgpp_idx)
            rela_conc = self.counts_to_molar * counts(states['bulk'],
                self.process.rela_idx)
            spot_conc = self.counts_to_molar * counts(states['bulk'],
            ...
            delta_metabolites, _, _, _, _, _ = self.ppgpp_metabolite_changes(
                ...
                self.counts_to_molar, v_rib, timestep, request=True
            )
        ...
        return fraction_charged, aa_counts_for_translation, {
            'bulk': [(self.process.charging_molecule_idx, requested_molecules),
                ...
                (self.process.ppgpp, counts(states['bulk'],
                    self.process.ppgpp)),
    ...
    def evolve(self, timestep, states, total_aa_counts, aas_used, next_amino_acid_count, nElongations, nInitialized):
        ...
        ## Net charged is tRNA that can be charged minus allocated charged
        # tRNA for uncharging
        aa_for_charging = total_aa_counts - aas_used
        n_aa_charged = np.fmin(aa_for_charging, np.dot(
            self.process.aa_from_trna, uncharged_trna))
        n_trna_charged = self.distribution_from_aa(
            n_aa_charged, uncharged_trna, True)

        # Reactions that are charged and elongated in same time step
        total_uncharging_reactions = self.distribution_from_aa(
            aas_used, total_trna)
        ...
        # Update proton counts to reflect polymerization reactions and transfer
        # of AA from tRNA. Peptide bond formation releases a water but
        # transferring AA from tRNA consumes a OH-. Net production of H+ for
        # each elongation, consume extra water for each initialization
        ...
        aa_diff = self.process.aa_supply - np.dot(self.process.aa_from_trna,
            total_charging_reactions)
        ...
        return net_charged, {aa: diff for aa, diff in zip(
            self.aaNames, aa_diff)}, update

    def calculate_trna_charging(self, synthetase_conc, uncharged_trna_conc,
        charged_trna_conc, aa_conc, ribosome_conc, f, time_limit=1000,
        use_disabled_aas=False):
        '''
        ...
        charged/uncharged is also used to determine how quickly the ribosome
        is elongating.

        Args:
            synthetase_conc (array of floats with concentration units) -
                concentration of synthetases associated with each amino acid
            uncharged_trna_conc (array of floats with concentration units) -
                concentration of uncharged tRNA associated with each amino acid
            charged_trna_conc (array of floats with concentration units) -
                concentration of charged tRNA associated with each amino acid
            aa_conc (array of floats with concentration units) - concentration
                of each amino acid
            ribosome_conc (float with concentration units) - concentration of
                active ribosomes
            f (array of floats) - fraction of each amino acid to be
                incorporated to total amino acids incorporated
            time_limit (float) - time limit to reach steady state
            use_disabled_aas (bool) - if True, all amino acids will be used for
                charging calculations, if False, some will be excluded as
                determined in initialize

        Returns:
            fraction_charged (array of floats) - fraction of total tRNA that is
                charged for each tRNA species
        ...
        def negative_check(trna1, trna2):
            ...
            bring concentration of trna1 to 0 and keep the same total
            concentration.

            Args:
                trna1 (ndarray[float]): concentration of one tRNA species 
                    (charged or uncharged)
                trna2 (ndarray[float]): concentration of another tRNA species
                    (charged or uncharged)
            '''
        ...
        def dcdt(c, t):
            ...
            Args:
                c (ndarray[float]): 1D array of concentrations of uncharged and
                    charged tRNAs. dims = 2*number of amino acids (uncharged
                    tRNA come first, then charged)
                t (float): time of integration step

            Returns:
                ndarray[float]: dc/dt for tRNA concentrations. dims = 2*number
                    of amino acids (uncharged tRNA come first, then charged)
            '''

            uncharged_trna_conc = c[:n_aas]
            charged_trna_conc = c[n_aas:]

            v_charging = (self.kS * synthetase_conc * uncharged_trna_conc *
                aa_conc / (self.KMaa * self.KMtf) / (1 + uncharged_trna_conc/
                    self.KMtf + aa_conc/self.KMaa + uncharged_trna_conc*
                    aa_conc/self.KMtf/self.KMaa))
            numerator_ribosome = 1 + np.sum(f * (self.krta / charged_trna_conc
                + uncharged_trna_conc / charged_trna_conc * self.krta /
                self.krtf))
            v_rib = (self.maxRibosomeElongationRate * ribosome_conc /
                numerator_ribosome)
        ...
        fraction_charged = charged_trna_conc / (uncharged_trna_conc +
            charged_trna_conc)
        numerator_ribosome = 1 + np.sum(f * (self.krta / charged_trna_conc +
            uncharged_trna_conc / charged_trna_conc * self.krta / self.krtf))
        v_rib = (self.maxRibosomeElongationRate * ribosome_conc /
            numerator_ribosome)
    ...
    def ppgpp_metabolite_changes(self, uncharged_trna_conc, charged_trna_conc,
        ...
        '''
        Calculates the changes in metabolite counts based on ppGpp synthesis
        ...
        Args:
            ...
            request (bool): if True, only considers reactant stoichiometry,
                ...
                and the request can be off the actual use if not handled in
                this manner.
            limits (np.array[float]): counts of molecules that are available to
                prevent negative total counts as a result of delta_metabolites.
                If None, no limits are placed on molecule changes.

        Returns:
            Tuple of:

            * delta_metabolites (np.array[int]): the change in counts of each
              metabolite involved in ppGpp reactions
        ...
        ribosomes_bound_to_uncharged[mask] = (ribosome_conc * f[mask] *
        ...
        v_deg = self.k_SpoT_deg * spot_conc * ppgpp_conc / (1 +
            uncharged_trna_conc.sum() / self.KI_SpoT)

        # Convert to discrete reactions
        n_syn_reactions = stochasticRound(self.process.random_state, v_syn *
            timestep / counts_to_micromolar)[0]
        n_deg_reactions = stochasticRound(self.process.random_state, v_deg *
            timestep / counts_to_micromolar)[0]
        ...
        if request:
            ...
            ppgpp_reaction_stoich[reactants] = self.ppgpp_reaction_stoich[
                reactants]
        else:
        ...
        for it in range(max_iterations):
            delta_metabolites = (ppgpp_reaction_stoich[:, self.synthesis_index]
                * n_syn_reactions + ppgpp_reaction_stoich[
                    :, self.degradation_index] * n_deg_reactions)
            ...
            if limits is None:
                ...
                if np.all(final_counts >= 0) or (old_counts is not None and
                    np.all(final_counts == old_counts)
                ):
                    break

                limited_index = np.argmin(final_counts)
                if ppgpp_reaction_stoich[
                    limited_index, self.synthesis_index] < 0:
                    limited = np.ceil(final_counts[limited_index] /
                        ppgpp_reaction_stoich[limited_index,
                            self.synthesis_index])
                    n_syn_reactions -= min(limited, n_syn_reactions)
                if ppgpp_reaction_stoich[
                    limited_index, self.degradation_index] < 0:
                    limited = np.ceil(final_counts[limited_index] / 
                        ppgpp_reaction_stoich[limited_index,
            ...
            raise ValueError('Failed to meet molecule limits with'
                ' ppGpp reactions.')

        return (delta_metabolites, n_syn_reactions, n_deg_reactions,
            v_rela_syn, v_spot_syn, v_deg)
