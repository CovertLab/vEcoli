...
def plot(
    ...
    rna_synth_prob_dict = {
        ...
        for i, rna in enumerate(
            get_field_metadata(
                conn, config_sql, "listeners__rna_synth_prob__actual_rna_synth_prob"
            )
        )
    }
    ...
    bulk_dict = {
        mol[:-3]: i
        for i, mol in enumerate(get_field_metadata(conn, config_sql, "bulk"))
    }
    ...
    metab_counts_dict = {
        ...
        for i, mol in enumerate(
            get_field_metadata(
                conn, config_sql, "listeners__enzyme_kinetics__metabolite_counts_final"
            )
        )
    }
    ...
    tf_binding_dict = {
        ...
        for i, tf in enumerate(
            get_field_metadata(
                conn, config_sql, "listeners__rna_synth_prob__new_n_binding_events"
            )
        )
    }
    ...
    purC_purR_monomers = named_idx(
        ...
        [purC_monomer_name, purR_monomer_name],
        [purC_monomer_idx, purR_monomer_idx],
    )
    purC_gene = named_idx(
    ...
    purC_rna_synth_prob = named_idx(
        ...
        [purC_rna_synth_prob_name],
        [purC_tu_idx],
    )
    purC_rna_synth_aff = named_idx(
        "listeners__rna_synth_prob__rna_synth_aff",
        [purC_rna_synth_aff_name],
        [purC_tu_idx],
    )
    purR_bulk = named_idx("bulk", purR_bulk_names, purR_bulk_idxs)
    hypoxanthine_metab_counts = named_idx(
        "listeners__enzyme_kinetics__metabolite_counts_final",
        [hypoxanthine_name],
        [hypoxanthine_metab_idx],
    )
    purR_tf_binding = named_idx(
        "listeners__rna_synth_prob__new_n_binding_events",
        [purR_tf_binding_name],
        [purR_tf_idx],
    )
    purR_tf_unbinding = named_idx(
        "listeners__rna_synth_prob__new_n_unbinding_events",
        [purR_tf_unbinding_name],
        [purR_tf_idx],
    )
    purR_tf_bound_binding_sites = named_idx(
        "listeners__rna_synth_prob__new_n_bound_binding_sites",
        [purR_tf_bound_binding_sites_name],
        [purR_tf_idx],
    )
    purR_tf_available_binding_sites = named_idx(
        "listeners__rna_synth_prob__new_n_available_binding_sites",
        [purR_tf_avail_binding_sites_name],
        [purR_tf_idx],
    )
    # purR_bound_to_purC = ndidx_to_duckdb_expr(
    #    "listeners__rna_synth_prob__new_n_bound_TF_per_TU", [purC_tu_idx, purR_tf_idx]
    # )
    ...
    purC_data = read_stacked_columns(
        ...
        [
            ...
            purC_rna_synth_aff,
            purR_bulk,
            hypoxanthine_metab_counts,
            purR_tf_binding,
            purR_tf_unbinding,
            purR_tf_bound_binding_sites,
            purR_tf_available_binding_sites,
        ],
        conn=conn,
    )
    ...
    cell_mass_data = read_stacked_columns(
        history_sql, ["listeners__mass__cell_mass"], conn=conn
    )
    ...
    cell_mass_dataframe = cell_mass_dataframe.with_columns(
        (
            cell_mass_dataframe["listeners__mass__cell_mass"]
            * (n_avogadro / cell_density).asNumber(units.L / (units.fg * units.mol))
        ).alias("counts_to_mols")
    )
    ...
    purC_dataframe = purC_dataframe.with_columns(
        purC_dataframe[hypoxanthine_name].cast(float)
        / cell_mass_dataframe["counts_to_mols"]
    )
    purC_dataframe = purC_dataframe.with_columns(
        (
            purC_dataframe[purC_rna_synth_aff_name]
            / purC_dataframe[purC_gene_name].cast(float)
        ).alias(purC_per_copy_synth_aff_name)
    )
    purC_dataframe = purC_dataframe.with_columns(
        (
            purC_dataframe[purC_rna_synth_prob_name]
            / purC_dataframe[purC_gene_name].cast(float)
        ).alias(purC_per_copy_synth_prob_name)
    )
    ...
    axs[3].plot(
        purC_dataframe["time"], purC_dataframe[purC_rna_synth_prob_name], label="total"
    )
    axs[3].plot(
        purC_dataframe["time"],
        purC_dataframe[purC_per_copy_synth_prob_name],
        label="per gene copy",
    )
    ...
    axs[4].plot(
        purC_dataframe["time"], purC_dataframe[purC_rna_synth_aff_name], label="total"
    )
    axs[4].plot(
        purC_dataframe["time"],
        purC_dataframe[purC_per_copy_synth_aff_name],
        label="per gene copy",
    )
    ...
    axs[5].plot(
        purC_dataframe["time"],
        purC_dataframe[purR_monomer_name],
        label=purR_monomer_name,
    )
    # TODO: check, if a purR is bound to DNA, is it still counted in bulk molecules?
    axs[5].plot(
        purC_dataframe["time"],
        purC_dataframe[purR_tf_bound_binding_sites_name],
        label=purR_tf_bound_binding_sites_name,
    )
    ...
    axs[7].plot(
        purC_dataframe["time"],
        purC_dataframe[purR_tf_binding_name],
        label=purR_tf_binding_name,
    )
    axs[7].plot(
        purC_dataframe["time"],
        purC_dataframe[purR_tf_unbinding_name],
        label=purR_tf_unbinding_name,
    )
    axs[7].legend()
    ...
    axs[8].plot(
        purC_dataframe["time"],
        purC_dataframe[purR_tf_avail_binding_sites_name],
        label=purR_tf_avail_binding_sites_name,
    )
    axs[8].legend()
