...
def plot(
    ...
    if len(new_gene_mRNA_ids) == 0:
        print("This plot requires simulations where the new gene option was "
    ...
    mRNA_idx_dict = {
        rna[:-3]: i for i, rna in enumerate(
    ...
    columns = {
        ...
        **named_idx('listeners__monomer_counts',
                    new_gene_monomer_ids, new_gene_monomer_indexes)
        **named_idx('listeners__rna_counts__mRNA_counts',
                    new_gene_mRNA_ids, new_gene_mRNA_indexes)
    }
    new_gene_data = history_lf.select(**columns).sort('Time (min)').collect(
        streaming=True)
