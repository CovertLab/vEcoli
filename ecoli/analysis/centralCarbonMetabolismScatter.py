...
class Plot:
    ...
    def do_plot(
        self, raw_data, arrData, sim_df, simDataFile, validationDataFile, outFile
    ):
        ...
        plt.text(
            x=0.5,
            y=0.97,
            s="$R^2$ = {:.3}, p = {:.0e}".format(correlation_coefficient**2, p).replace(
                "e-0", "e-"
            ),
...
def get_toya_flux_rxns(simDataFile):
    ...
    normal_reaction_mapping = {
        ...
        "6PFRUCTPHOS-RXN": {"FRUCTOSE-6P[c]": -1, "FRUCTOSE-16-DIPHOSPHATE[c]": 1},
        "F16ALDOLASE-RXN": {
            "FRUCTOSE-16-DIPHOSPHATE[c]": -1,
            "DIHYDROXY-ACETONE-PHOSPHATE[c]": 1,
            "GAP[c]": 1,
        },
        "FUMHYDR-RXN": {"FUM[c]": -1, "MAL[c]": 1},
        "ISOCITDEH-RXN": {
            "THREO-DS-ISO-CITRATE[c]": -1,
            "2-KETOGLUTARATE[c]": 1,
            "CARBON-DIOXIDE[c]": 1,
        },
        "MALATE-DEH-RXN": {"MAL[c]": -1, "OXALACETIC_ACID[c]": 1},
        "PEPCARBOX-RXN": {"PHOSPHO-ENOL-PYRUVATE[c]": -1, "OXALACETIC_ACID[c]": 1},
        "PEPDEPHOS-RXN": {"PHOSPHO-ENOL-PYRUVATE[c]": -1, "PYRUVATE[c]": 1},
        "PGLUCISOM-RXN": {"D-glucopyranose-6-phosphate[c]": -1, "FRUCTOSE-6P[c]": 1},
        "PYRUVDEH-RXN": {"PYRUVATE[c]": -1, "ACETYL-COA[c]": 1, "CARBON-DIOXIDE[c]": 1},
        "RIB5PISOM-RXN": {"RIBULOSE-5P[c]": -1, "RIBOSE-5P[c]": 1},
        "RIBULP3EPIM-RXN": {"RIBULOSE-5P[c]": -1, "XYLULOSE-5-PHOSPHATE[c]": 1},
        "RXN-9952": {"CPD-2961[c]": -1, "RIBULOSE-5P[c]": 1, "CARBON-DIOXIDE[c]": 1},
        ...
        "TRIOSEPISOMERIZATION-RXN": {"DIHYDROXY-ACETONE-PHOSPHATE[c]": -1, "GAP[c]": 1},
    }
    rxn_ids = {}
    ...
    step_skip_rxn_mapping = {
        ...
        "CITSYN-RXN": {
            ...
            1: {"OXALACETIC_ACID[c]": -1, "ACETYL-COA[c]": -1, "CIT[c]": 1},
            2: {"CIT[c]": -1, "THREO-DS-ISO-CITRATE[c]": 1},
        },
        "2PGADEHYDRAT-RXN": {
            # Toya goes directly from 3-PG (G3P in model) to PEP
            1: {"G3P[c]": -1, "2-PG[c]": 1},
            2: {"2-PG[c]": -1, "PHOSPHO-ENOL-PYRUVATE[c]": 1},
        },
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sim_data", "-s", help="Path to sim_data file.", default=SIM_DATA_PATH
    )
    parser.add_argument(
        "--validation_data",
        "-v",
    ...
    sim_df = pd.read_csv(args.sim_df, dtype={"Agent ID": str, "Seed": str}, index_col=0)
    ...
    plot = Plot()
    plot.do_plot(
        raw_data, arr_data, sim_df, args.sim_data, args.validation_data, args.out_file
    )
