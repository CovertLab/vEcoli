    ...
    def do_plot(self, data):
        ...
        expectedCountsArbitrary = normalize(
            ...
            * self.sim_data.process.translation.translation_efficiencies_by_monomer
            / (
                np.log(2) / self.sim_data.doubling_time.asNumber(units.s)
                + self.sim_data.process.translation.monomer_data["deg_rate"].asNumber(
                    1 / units.s
                )
            )
        )
        ...
        maxLine = 1.1 * max(
            np.log10(expectedCountsRelative.max() + 1),
            np.log10(relativeCounts.max() + 1),
        )
        plt.plot([0, maxLine], [0, maxLine], "--r")
        plt.plot(
            np.log10(expectedCountsRelative + 1),
        ...
        plt.title(
            "PCC (of log values): %0.2f"
            % pearsonr(
                np.log10(expectedCountsRelative + 1), np.log10(relativeCounts + 1)
            )[0]
        )
