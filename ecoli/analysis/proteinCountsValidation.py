...
class Plot:
    def do_plot(self, monomer_counts, simDataFile, validationDataFile, outFile):
        ...
        ax[0].scatter(
            val_wisniewski_log, sim_wisniewski_log, c="k", edgecolor="k", alpha=0.05
        )
        ...
        r_over_30 = pearsonr(sim_wisniewski_log[over_30], val_wisniewski_log[over_30])[
            0
        ]
        r_under_30 = pearsonr(
        ...
        ax[0].text(
            x=0.5,
            y=1.1,
            s="count $\\geq$ 30: $R^2$ = %0.3f, n = %i" % (r_over_30**2, over_30.sum()),
        ...
        ax[1].scatter(
            val_schmidt_log, sim_schmidt_log, c="k", edgecolor="k", alpha=0.05
        )
        ...
        r_over_30 = pearsonr(sim_schmidt_log[over_30], val_schmidt_log[over_30])[0]
        r_under_30 = pearsonr(sim_schmidt_log[~over_30], val_schmidt_log[~over_30])[0]
        ax[1].text(
            x=0.5,
            y=1,
            s="count $\\geq$ 30: $R^2$ = %0.3f, n = %i" % (r_over_30**2, over_30.sum()),
...
def save_agent_avgs(data):
    ...
    for time, time_data in data.items():
        for agent_id, agent_data in time_data["agents"].items():
            ...
            agent_monomer_counts[agent_id].append(
                agent_data["listeners"]["monomer_counts"]
            )
...
if __name__ == "__main__":
    ...
    plot = Plot()
    plot.do_plot(
        agent_monomer_counts, args.sim_data, args.validation_data, args.out_file
    )
