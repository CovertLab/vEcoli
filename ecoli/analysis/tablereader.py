...
from ecoli.analysis.tablereader_utils import camel_case_to_underscored
...
MAPPING = {
    ...
    "RibosomeData": {
        "aaCountInSequence": ("listeners", "ribosome_data", "aa_count_in_sequence"),
        "aaCounts": ("listeners", "ribosome_data", "aa_counts"),
        "actualElongationHist": (
            "listeners",
            "ribosome_data",
            "actual_elongation_hist",
        ),
        ...
        "elongationsNonTerminatingHist": (
            "listeners",
            "ribosome_data",
            "elongations_non_terminating_hist",
        ),
        ...
        "translationSupply": ("listeners", "ribosome_data", "translation_supply"),
    ...
    "FBAResults": {
        ...
        "constrained_molecules": ("listeners", "fba_results", "constrained_molecules"),
        "deltaMetabolites": ("listeners", "fba_results", "delta_metabolites"),
        "targetConcentrations": ("listeners", "fba_results", "target_concentrations"),
        ...
        "uptake_constraints": ("listeners", "fba_results", "uptake_constraints"),
    },
    "MonomerCounts": {
...
class TableReader(object):
    ...
    def readColumn(
        ...
        if callable(viv_path):
            ...
            warn(
                f'No explicit mapping defined from {self._path + "/" + name} to a path in vivarium data,\n'
...
def _check_bulk_inputs(mol_names):
    ...
    for names in mol_names:
        if isinstance(names, ANY_STRING):
            raise Exception(
                "mol_names tuple must contain arrays not strings like {!r}".format(
                    names
                )
            )
...
def test_table_reader():
    ...
    sim.run()
    data = sim.query()
    emitted_config = {"state": sim.ecoli_experiment.state.get_config()}
