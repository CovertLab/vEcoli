...
def make_plot(datasets):
    ...
    fig = plt.figure(figsize=(
        n_cols * COL_WIDTH + LEGEND_WIDTH, n_rows * ROW_HEIGHT))
    ...
    for path_idx, path in enumerate(paths):
        ...
        for key, ts in datasets.items():
            ...
            if path_idx == len(paths) - 1:
                ax.plot(
                    times, data, label=key, alpha=ALPHA)
            else:
...
def main(cli_args=None):
    ...
    parser.add_argument(
        '--out', '-o',
        default=f'{OUT_DIR}comparison.png',
        help='Output path')
    ...
    datasets = {
        experiment_id: path_timeseries_from_data(
            access(experiment_id, paths)[0])
