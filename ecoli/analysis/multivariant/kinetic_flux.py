...
def plot(
    ...
    Args:
        params: Dictionary of parameters given under analysis
            ...
            .. code-block:: json
                ...
                {
                    // Whether kinetic-reaction listener fields are raw
                    // counts requiring conversion to molar via
                    // counts_to_molar (true for MetabolismReduxClassic,
                    // false for the stock Metabolism process, which
                    // already emits molar flux and restricts kinetic
                    // fields to the kinetic-reaction subset).
                    "is_redux": true,
    ...
    is_redux = params.get("is_redux", True)
    ...
    if is_redux:
        ...
        kinetic_rxn_names = field_metadata(
            conn, config_sql, "listeners__enzyme_kinetics__target_fluxes"
        )
        kinetic_indices = None
    ...
    if is_redux:
        ...
        raw_columns = [
            "listeners__enzyme_kinetics__target_fluxes AS target_kinetic_fluxes",
            "listeners__enzyme_kinetics__actual_fluxes AS estimated_fluxes",
        ]
    ...
    estimated_arr = ndlist_to_ndarray(raw["estimated_fluxes"])
    if is_redux:
        # estimated_fluxes covers all reactions; subset to the kinetic ones.
        ...
        # enzyme_kinetics fields are already restricted to kinetic reactions
        # and already in molar-flux units (mmol/(L·s)); just convert to
        # mmol/(L·h) to match the redux-classic branch's units.
        kinetic_flux_arr = estimated_arr
        counts_to_molar = np.array(S_PER_HR)
    ...
    for v in unique_variants:
        ...
        mask = variants_col == v
        ctm = counts_to_molar[mask] if is_redux else counts_to_molar
    ...
    # ── Line-plot DataFrame ────────────────────────────────────────────────────
    # "Unweighted Kinetic Term" is computed here from the target/flux arrays
    # already loaded above (sum of |flux - target| across kinetic reactions
    # per timestep), rather than read from a listener column: neither
    # metabolism.py nor the current metabolism_redux_classic.py emits a
    # "kinetics_term" field (that name belongs to a different, third process
    # file, metabolism_redux.py), so deriving it here works uniformly for
    # both processes without depending on process-specific listeners.
    kinetics_term_arr = np.sum(
        np.abs(kinetic_flux_arr - target_arr) * counts_to_molar, axis=1
    )
    variant_label_col = [variant_label_map[v] for v in raw["variant"].to_list()]
    line_df = (
        raw.select(["Time (min)", "generation", "lineage_seed"])
        .with_columns(
            pl.Series("Variant", variant_label_col),
            pl.Series("kinetics_term", kinetics_term_arr),
        )
        .to_pandas()
    )
