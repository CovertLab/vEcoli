...
def plot(
        ...
        params: Dictionary of parameters given under analysis
            ...
            .. code-block:: json

                {
                    ...
                    // MetabolismReduxClassic, false for the stock
                    // Metabolism process, which already emits molar flux).
                    "is_redux": true
                }
    """
    ...
    if missing_catalysts:
        ...
        for cat in missing_catalysts:
            requested_catalysts.remove(cat)
    ...
    if is_redux:
        columns.append("listeners__enzyme_kinetics__counts_to_molar AS counts_to_molar")
    ...
    # Redux classic emits base_reaction_fluxes as raw counts; convert to
    # molar flux via counts_to_molar. The stock Metabolism process already
    # emits molar flux, so no conversion is needed there.
    if is_redux:
        raw = raw.with_columns(
            [(pl.col(r) * pl.col("counts_to_molar")).alias(r) for r in all_reactions]
        )
    flux_axis_title = "Base reaction flux (mmol/L/s)"
    ...
    for variant_val in variants:
        ...
        flux_chart = (
            ...
            .encode(
                x=alt.X("Time (min):Q", title="Time (min)"),
                y=alt.Y("mean(flux):Q", title=flux_axis_title),
                color=alt.Color(
                    "catalyst:N", scale=color_scale, legend=alt.Legend(title="Catalyst")
                ),
