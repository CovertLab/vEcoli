...
def plot(
        ...
        params: Dictionary of parameters given under analysis
            ...
            .. code-block:: json

                {
                    ...
                    // Whether the sim was run with MetabolismReduxClassic
                    // (true, reads listeners__fba_results__solution_fluxes)
                    // or the stock Metabolism process (false, reads
                    // listeners__fba_results__reaction_fluxes). Both are
                    // already in molar-flux units, so no counts_to_molar
                    // conversion is applied either way; only the field name
                    // differs. Note that min_abs_contrib is in native flux
                    // units for whichever process is selected, so a
                    // threshold tuned for one process's magnitude may need
    ...
    flux_field = (
        ...
        if is_redux
        else "listeners__fba_results__reaction_fluxes"
    )
    ...
    solution_flux_ids = field_metadata(conn, config_sql, flux_field)
