...
def plot(
    ...
    data = history_lf.select({
        ...
        **named_idx('bulk', [f'complex__{i}' for i in complex_ids],
            [bulk_ids.index(i) for i in complex_ids]),
        **named_idx('bulk', [f'uncharged_tRNA__{i}' for i in uncharged_tRNA_ids],
            [bulk_ids.index(i) for i in uncharged_tRNA_ids]),
        **named_idx('bulk', [f'charged_tRNA__{i}' for i in charged_tRNA_ids],
            [bulk_ids.index(i) for i in charged_tRNA_ids]),
        **named_idx('bulk', [f'rRNA__{i}' for i in rRNA_ids],
            [bulk_ids.index(i) for i in rRNA_ids]),
        **named_idx('bulk', [f'ribosomal_subunit__{i}' for i in ribosomal_subunit_ids],
            [bulk_ids.index(i) for i in ribosomal_subunit_ids]),
    }).with_columns([
        pl.col('time').rank('min').over(
            ['lineage_seed', 'generation', 'agent_id'])
            .alias('rank')
    # Filter out first timestep for each cell because counts_to_molar is 0
    ]).filter(pl.col("rank") != 1).drop("rank").collect(streaming=True)

    # Calculate statistics for gene copy numbers
    gene_copy_numbers = data['gene_copy_numbers'].list.to_struct(
        fields=gene_ids_rna_synth_prob).struct.unnest()
