...
def create_bulk_container(sim_data, n_seeds=1, condition=None, seed=0,
                          ppgpp_regulation=True, trna_attenuation=True,
                          mass_coeff=1, form_complexes=True):
    try:
...
def initialize_bulk_counts(sim_data, media_id, import_molecules, random_state,
    ...
    bulk_counts = np.zeros(len(
        sim_data.internal_state.bulk_molecules.bulk_data["id"]), dtype=int)

    # Set protein counts from expression
    initialize_protein_monomers(bulk_counts, sim_data, random_state, mass_coeff,
                                ppgpp_regulation, trna_attenuation)

    #Set RNA counts from expression
    initialize_rna(bulk_counts, sim_data, random_state, mass_coeff, ppgpp_regulation,
                   trna_attenuation)
    ...
    set_small_molecule_counts(bulk_counts, sim_data, media_id,
                              import_molecules, mass_coeff)
    ...
    bulk_array = np.array([mol_data for mol_data in
                           zip(bulk_ids, bulk_counts, *bulk_submasses)],
...
def initialize_unique_molecules(bulk_state, sim_data, cell_mass, random_state,
                                unique_id_rng, superhelical_density,
                                ppgpp_regulation, trna_attenuation,
                                mechanistic_replisome, kinetic_trna_charging):
    unique_molecules = {}
    ...
    initialize_replication(bulk_state, unique_molecules, sim_data,
                           cell_mass, mechanistic_replisome, unique_id_rng)

    # Initialize bound transcription factors
    initialize_transcription_factors(bulk_state, unique_molecules, sim_data,
                                     random_state)

    # Initialize active RNAPs and unique molecule representations of RNAs
    initialize_transcription(bulk_state, unique_molecules, sim_data, random_state,
                             unique_id_rng, ppgpp_regulation, trna_attenuation)

    # Initialize linking numbers of chromosomal segments
    if superhelical_density:
        initialize_chromosomal_segments(unique_molecules, sim_data,
                                        unique_id_rng)
    else:
    ...
    initialize_translation(bulk_state, unique_molecules, sim_data, random_state,
                           unique_id_rng, kinetic_trna_charging)
...
def create_new_unique_molecules(name, n_mols, sim_data, random_state, **attrs):
    ...
    unique_mols["unique_index"] = create_unique_indexes(
        n_mols, random_state)
    unique_mols["_entryState"] = 1
    return unique_mols


def initialize_protein_monomers(bulk_counts, sim_data, random_state,
                                mass_coeff, ppgpp_regulation,
                                trna_attenuation):
    monomer_mass = mass_coeff * sim_data.mass.get_component_masses(
        sim_data.condition_to_doubling_time[sim_data.condition])["proteinMass"
    ...
    monomer_expression = normalize(
        sim_data.process.transcription.cistron_tu_mapping_matrix.dot(
            rna_expression)[sim_data.relation.cistron_to_monomer_mapping] *
        sim_data.process.translation.translation_efficiencies_by_monomer /
        (np.log(2) / sim_data.condition_to_doubling_time[
            sim_data.condition].asNumber(units.s) + 
    ...
    n_monomers = countsFromMassAndExpression(
        ...
        sim_data.process.translation.monomer_data["mw"].asNumber(
            units.g / units.mol),
        monomer_expression,
    ...
    bulk_counts[monomer_idx] = random_state.multinomial(
        n_monomers, monomer_expression)


def initialize_rna(bulk_counts, sim_data, random_state, mass_coeff,
                   ppgpp_regulation, trna_attenuation):
    """
    ...
    rna_mass = mass_coeff * sim_data.mass.get_component_masses(
        sim_data.condition_to_doubling_time[sim_data.condition])["rnaMass"
        ] / sim_data.mass.avg_cell_to_initial_cell_conversion_factor
    ...
    bulk_counts[rna_idx] = random_state.multinomial(
        n_rnas, rna_expression)
...
def set_small_molecule_counts(bulk_counts, sim_data, media_id,
                              import_molecules, mass_coeff, cell_mass=None):
    ...
    bulk_ids = sim_data.internal_state.bulk_molecules.bulk_data["id"]
    molecule_concentrations = (units.mol / units.L) * np.array([conc_dict[
        key].asNumber(units.mol / units.L) for key in molecule_ids])

    if cell_mass is None:
        avg_cell_fraction_mass = sim_data.mass.get_component_masses(
            doubling_time)
        other_dry_mass = (mass_coeff * (avg_cell_fraction_mass["proteinMass"]
        ...
        for mol in conc_dict:
            ...
            small_molecule_mass += (bulk_counts[mol_idx]
...
def initialize_complexation(bulk_counts, sim_data, random_state):
    ...
    updated_molecule_counts, complexation_events = \
        mccFormComplexesWithPrebuiltMatrices(
            molecule_counts,
...
def initialize_full_chromosome(unique_molecules, sim_data, unique_id_rng):
    ...
    unique_molecules["full_chromosome"] = create_new_unique_molecules(
        "full_chromosome", 1, sim_data, unique_id_rng,
        division_time=0.0, has_triggered_division=True, domain_index=0)


def initialize_replication(bulk_state, unique_molecules, sim_data, cell_mass,
                           mechanistic_replisome, unique_id_rng):
    """
    ...
    unique_molecules["oriC"] = create_new_unique_molecules(
        "oriC", n_oric, sim_data, unique_id_rng,
    ...
    unique_molecules["chromosome_domain"] = create_new_unique_molecules(
        "chromosome_domain", n_domain, sim_data, unique_id_rng,
    ...
    if n_replisome != 0:
        ...
        if mechanistic_replisome:
            ...
            replisome_mass_array = 3*replisome_trimer_subunit_masses.sum(
                axis=0) + replisome_monomer_subunit_masses.sum(axis=0)
    ...
    unique_molecules["promoter"] = create_new_unique_molecules(
        "promoter", n_promoter, sim_data, unique_id_rng,
    ...
    unique_molecules["gene"] = create_new_unique_molecules(
        "gene", n_gene, sim_data, unique_id_rng,
    ...
    unique_molecules["DnaA_box"] = create_new_unique_molecules(
        "DnaA_box", n_DnaA_box, sim_data, unique_id_rng,
        domain_index=DnaA_box_domain_index,
        coordinates=DnaA_box_coordinates,
        DnaA_bound=np.zeros(n_DnaA_box, dtype=bool))


def initialize_transcription_factors(bulk_state, unique_molecules, sim_data,
                                     random_state):
    """
    ...
    tf_indexes = [np.where(bulk_state["id"] == tf_id + "[c]")[0][0]
        for tf_id in tf_ids]
    ...
    for tf_idx, tf_id in enumerate(tf_ids):
        ...
        if tf_to_tf_type[tf_id] == "0CS":
            ...
            p_promoter_bound = p_promoter_bound_TF(
        ...
        n_to_bind = int(stochasticRound(random_state, np.full(
            n_available_promoters, p_promoter_bound)).sum())
...
def initialize_transcription(bulk_state, unique_molecules, sim_data,
                             random_state, unique_id_rng, ppgpp_regulation,
                             trna_attenuation):
    """
    ...
    rna_masses = (sim_data.process.transcription.rna_data["mw"] / 
    ...
    frac_active_rnap = sim_data.process.transcription.rnapFractionActiveDict[
        current_media_id]
    inactive_rnap_idx = bulk_name_to_idx(sim_data.molecule_ids.full_RNAP,
                                         bulk_state["id"])
    ...
    delta_prob_matrix = \
        sim_data.process.transcription_regulation.get_delta_prob_matrix(
    ...
    if len(perturbations) > 0:
        probability_indexes = [
            (index, sim_data.genetic_perturbations[rna_data["id"]])
                for index, rna_data in enumerate(
    ...
    idx_rprotein = np.where(sim_data.process.transcription.rna_data[
        "includes_ribosomal_protein"])[0]
    idx_rnap = np.where(sim_data.process.transcription.rna_data[
        "includes_RNAP"])[0]
    ...
    promoter_init_probs[is_mRNA] *= (synth_prob_fractions["mRna"]
                                     / promoter_init_probs[is_mRNA].sum())
    promoter_init_probs[is_tRNA] *= (synth_prob_fractions["tRna"]
                                     / promoter_init_probs[is_tRNA].sum())
    promoter_init_probs[is_rRNA] *= (synth_prob_fractions["rRna"]
                                     / promoter_init_probs[is_rRNA].sum())
    ...
    if trna_attenuation:
        attenuation_readthrough = {
            ...
            zip(sim_data.process.transcription.attenuated_rna_indices,
                sim_data.process.transcription.attenuation_readthrough[
                    sim_data.condition])
            }
        readthrough_adjustment = np.array([attenuation_readthrough.get(idx, 1)
                                           for idx in TU_index])
        promoter_init_probs *= readthrough_adjustment

    scale_the_rest_by = (1. - promoter_init_probs[is_fixed].sum()
    ...
    init_prob_normalized = (init_prob_length_adjusted
                            / init_prob_length_adjusted.sum())

    # Sample a multinomial distribution of synthesis probabilities to determine
    # what RNA are initialized
    n_initiations = random_state.multinomial(
    ...
    updated_lengths = np.array(random_state.rand(n_RNAPs_to_activate)
    ...
    unique_molecules["active_RNAP"] = create_new_unique_molecules(
        "active_RNAP", n_RNAPs_to_activate, sim_data, unique_id_rng,
        domain_index=domain_index_rnap,
        coordinates=updated_coordinates,
        is_forward=is_forward)

    # Decrement counts of bulk inactive RNAPs
    rnap_idx = bulk_name_to_idx(sim_data.molecule_ids.full_RNAP,
                                bulk_state["id"])
    ...
    partial_rnas = create_new_unique_molecules(
        "RNA", n_RNAPs_to_activate, sim_data, unique_id_rng,
    ...
    full_rnas = create_new_unique_molecules(
        "RNA", len(TU_index_full_mRNAs), sim_data, unique_id_rng,
...
def initialize_chromosomal_segments(unique_molecules, sim_data, unique_id_rng):
    ...
    (replisome_coordinates, replisome_domain_indexes,
     replisome_unique_indexes) = attrs(
        unique_molecules["active_replisome"], 
        ["coordinates", "domain_index", "unique_index"])

    (active_RNAP_coordinates, active_RNAP_domain_indexes,
     active_RNAP_unique_indexes) = attrs(unique_molecules["active_RNAP"],
        ['coordinates', 'domain_index', 'unique_index'])

    chromosome_domain_domain_indexes, child_domains = attrs(
        unique_molecules["chromosome_domain"],
    ...
    for domain_index in chromosome_domain_domain_indexes:
        ...
        molecule_coordinates_this_domain = active_RNAP_coordinates[
            RNAP_domain_mask]
        molecule_indexes_this_domain = active_RNAP_unique_indexes[
            RNAP_domain_mask]
        ...
        (boundary_molecule_indexes_this_domain, 
         ...
         linking_numbers_this_domain) = get_chromosomal_segment_attributes(
    ...
    assert n_segments == len(active_RNAP_unique_indexes) + 1.5*len(
        replisome_unique_indexes) + 1

    # Add chromosomal segments
    unique_molecules["chromosomal_segment"] = create_new_unique_molecules(
        "chromosomal_segment", n_segments, sim_data, unique_id_rng,
...
def initialize_translation(bulk_state, unique_molecules, sim_data,
                           random_state, unique_id_rng, kinetic_trna_charging):
    """
    ...
    protein_lengths = sim_data.process.translation.monomer_data[
        'length'].asNumber()
    translation_efficiencies = normalize(
        sim_data.process.translation.translation_efficiencies_by_monomer)
    aa_weights_incorporated = \
        sim_data.process.translation.translation_monomer_weights
    ...
    cistron_lengths = sim_data.process.transcription.cistron_data[
        'length'].asNumber(units.nt)
    ...
    monomer_index_to_cistron_index = {
        i: sim_data.process.transcription._cistron_id_to_index[
            monomer['cistron_id']]
        for (i, monomer) in enumerate(sim_data.process.translation.monomer_data)
        }

    # Get attributes of RNAs
    (TU_index_all_RNAs, length_all_RNAs, is_mRNA, is_full_transcript_all_RNAs,
     unique_index_all_RNAs) = attrs(unique_molecules["RNA"],
        ['TU_index', 'transcript_length', 'is_mRNA', 'is_full_transcript', 
        'unique_index'])
    ...
    TU_index_incomplete_mRNAs = TU_index_mRNAs[
        np.logical_not(is_full_transcript_mRNAs)]
    length_incomplete_mRNAs = length_mRNAs[
        np.logical_not(is_full_transcript_mRNAs)]
    ...
    ribosome30S_idx = bulk_name_to_idx(sim_data.molecule_ids.s30_full_complex,
                                       bulk_state["id"])
    ribosome30S = bulk_state["count"][ribosome30S_idx]
    ribosome50S_idx = bulk_name_to_idx(sim_data.molecule_ids.s50_full_complex,
                                       bulk_state["id"])
    ...
    n_ribosomes_to_activate = np.int64(
        frac_active_ribosome * inactive_ribosome_count)
    ...
    cistron_start_positions_on_mRNA = np.empty(
        n_ribosomes_to_activate, np.int64)
    ...
    for protein_index, counts in zip(
        ...
        positions_on_mRNA_from_cistron_start_site[start_index:start_index+counts] = np.floor(
            random_state.rand(counts) * np.repeat(
    ...
    unique_molecules["active_ribosome"] = create_new_unique_molecules(
        "active_ribosome", n_ribosomes_to_activate, sim_data, unique_id_rng,
...
def determine_chromosome_state(tau, replichore_length, n_max_replisomes,
    ...
    n_rounds = min(n_max_rounds, max(
        0, int(np.ceil(np.log2(unitless_cell_mass / unitless_critical_mass)))))
    ...
    domain_index_oric = np.arange(2**n_rounds - 1, 2**(n_rounds + 1) - 1,
                                  dtype=np.int32)
    ...
    for round_idx in np.arange(n_rounds):
        ...
        replication_time = np.log(unitless_cell_mass / round_critical_mass
                                  ) / growth_rate
        ...
        fork_location = min(np.floor(replication_time * replication_rate),
        ...
        coordinates[
            ...
            ] = np.tile(np.array([fork_location, -fork_location]),
                        n_events_this_round)
...
def calculate_cell_mass(bulk_state, unique_molecules, sim_data):
    ...
    """
    bulk_submass_names = [f'{submass}_submass' for submass in
                      sim_data.submass_name_to_index.keys()]
    cell_mass = bulk_state['count'].dot(rfn.structured_to_unstructured(
        bulk_state[bulk_submass_names])).sum()

    if len(unique_molecules) > 0:
        unique_masses = sim_data.internal_state.unique_molecule.\
            unique_molecule_masses['mass'].asNumber(units.fg / units.mol
                ) / sim_data.constants.n_avogadro.asNumber(1 / units.mol)
        unique_ids = sim_data.internal_state.unique_molecule.\
            unique_molecule_masses['id']
        unique_submass_names = [f'massDiff_{submass}' for submass in
                        sim_data.submass_name_to_index.keys()]
        for unique_id, unique_submasses in zip(unique_ids, unique_masses):
            if unique_id in unique_molecules:
                ...
                cell_mass += rfn.structured_to_unstructured(
                    unique_molecules[unique_id][unique_submass_names]).sum()
    
    return units.fg * cell_mass


def initialize_steady_state_trna_charging(bulk_state, unique_molecules, sim_data, 
                                          variable_elongation):
    '''
...
def initialize_kinetic_trna_charging(bulk_molecules, sim_data):
    '''
    ...
    # Fraction free
    f_free = [get_f_free(trna, sim_data.condition) for trna in free_trnas]
